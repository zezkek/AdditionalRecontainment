...
namespace AdditionalRecontainment.Commands
{
    ...
    class MTFRequest : ICommand
    {
        ...
        {
            Log.Info(ReadyToEvacuate.Count.ToString());
            sbyte ChopperCapacity = 10;
            //List<Pickup> ItemsToEvac = Handlers.items.Where(x => Vector3.Distance(x.position, midpos) <= Plugin.PluginItem.Config.Distance && Plugin.PluginItem.Config.MTFEvacItems.Contains(x.ItemId)).ToList();
        ...
        private IEnumerator<float> WaitingRoom()
        {
            ...
            List<Player> ReadyToEvac = Player.List.Where(x => Vector3.Distance(x.Position, midpos) <= Plugin.PluginItem.Config.Distance).ToList();
